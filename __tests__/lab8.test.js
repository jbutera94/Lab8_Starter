...
describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const btn = await shadowRoot.$('button');
    await btn.click();
    const btnInnerText = await btn.getProperty('innerText');
    const btnInnerTextValue = btnInnerText['_remoteObject'].value;
    expect(btnInnerTextValue).toBe('Remove from Cart');
  }, 2500);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const cartCountValue = await cartCountInnerText['_remoteObject'].value;
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < productItems.length; i++) {
      ...
      const btn = await shadowRoot.$('button');
      const btnInnerText = await btn.getProperty('innerText');
      const btnValue = await btnInnerText['_remoteObject'].value;
      expect(btnValue).toBe('Remove from Cart');
    }
    const cartCount = await page.$('#cart-count');
    const cartCountInnerText = await cartCount.getProperty('innerText');
    const cartCountValue = await cartCountInnerText['_remoteObject'].value;
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    const localStorageCart = await page.evaluate(() => window.localStorage.getItem('cart'));
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < productItems.length; i++) {
      ...
      const btnInnerText = await btn.getProperty('innerText');
      const btnValue = await btnInnerText['_remoteObject'].value;
      expect(btnValue).toBe('Add to Cart');
    }
    const cartCount = await page.$('#cart-count');
    const cartCountInnerText = await cartCount.getProperty('innerText');
    const cartCountValue = await cartCountInnerText['_remoteObject'].value;
